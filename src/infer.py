...
def infer(model_name, num_samples):
    ...
    for i in idx:
        ...
        # Save image
        image = image_proc.convert('RGB') # Make sure the imae is in RGB
        image_fn = os.path.join(processed_image_path, f"test_{i}.png")
    ...
    for im in os.listdir(processed_image_path):
        ...
        with torch.no_grad():
            ...
            print(classes[int(predictions)], im)
            # if (int(predictions.cpu()) == int(category)):
            #     num_correct += 1
